...
class Reposter:
    ...
    def post_to_bluesky(self, toot):
        posts = PostSplitter(TootParser(toot).text_builder).split()

        reply_ref = None
        in_thread = (toot.in_reply_to_id and toot.in_reply_to_id == self._last_reposted_toot_id)
        media_posted = False
        for post in posts:
            if in_thread and self._last_post_ref and self._last_root_post_ref:
                reply_ref = bluesky_models.AppBskyFeedPost.ReplyRef(parent=self._last_post_ref,
                                                                    root=self._last_root_post_ref)

            if toot.media_attachments and not media_posted:
                media_posted = True
                images = []
                image_alts = []
                ...
                if images:
                    response = self._bluesky.send_images(text=post.text,
                                                         facets=post.facets,
                                                         images=images,
                ...
                response = self._bluesky.send_post(text=post.text, facets=post.facets, reply_to=reply_ref)
